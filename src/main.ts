...
function getTestType(): TestType {
	const params: string[] = ((<HTMLInputElement>document.querySelector(`input[name="typePicker"]:checked`)).value).split("-");
	return {
		type: params[0],
		words: parseInt(params[1]),
		timer: parseInt(params[2])
	};
}
function finishTest() { // Ends the test
	...
	window.setTimeout(() => { // delay 125ms to make sure we get our last cpm/wpm update
...
function updateDisplays() { // Update the end displays below the input box
	...
	document.getElementById("timeLeft").innerHTML=isNaN(currTime-firstTime) || currentTestType.timer == 0 ? "0.0" : (currentTestType.timer-(currTime-firstTime)/1000).toFixed(1); // Time left (s), fix to 1 decimal place after the .
